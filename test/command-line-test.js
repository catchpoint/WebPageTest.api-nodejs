...
function getHelp(command) {
  ...
  return fs.readFileSync(
    path.join(__dirname, "./fixtures/command-line/help" + command + ".txt"),
    "utf8"
  );
}
...
describe("WebPageTest Command Line", function () {
  ...
  it("gets a test results for multi runs with custom median metric input returns the API url", function (done) {
    exec(mock("results 120816_V2_2 -m TTFB"), function (err, data) {
      if (err) return done(err);
      data = JSON.parse(data);
      assert.equal(data.url, wptServer + "jsonResult.php?test=120816_V2_2&medianMetric=TTFB");
      done();
    });
  });
  ...
  it("gets a test results with extra data input returns the API url", function (done) {
    exec(mock("results 141106_8N_ZRC -bDpR"), function (err, data) {
      ...
      assert.equal(
        data.url,
        wptServer + "jsonResult.php?test=141106_8N_ZRC&breakdown=1&domains=1&pagespeed=1&requests=1"
      );
      done();
    });
  });
    ...
    exec(
      ...
      function (err, data) {
        ...
        assert.equal(
          data.url,
          wptServer +
            "runtest.php?url=http%3A%2F%2Ftwitter.com%2Fmarcelduran&location=Local_Firefox_Chrome%3AChrome&runs=3&fvonly=1&label=test%20123&timeline=1&netlog=1&pngss=1&f=json"
        );
        done();
      }
    );
  ...
  it("gets a waterfall thumbnail input returns the API url", function (done) {
    exec(mock("waterfall 120816_V2_2 --thumbnail"), function (err, data) {
      ...
      assert.equal(
        data.url,
        wptServer + "thumbnail.php?test=120816_V2_2&run=1&cached=0&file=1_waterfall.png"
      );
      done();
    });
  });
  ...
  it("gets a customized waterfall image input returns the API url", function (done) {
    exec(
      mock(
        "waterfall 120816_V2_2 --type connection --mime --width 640 --max 9 --requests 1,2,4,6-8 --nocpu --nobandwidth --noellipsis --nolabels"
      ),
      function (err, data) {
        if (err) return done(err);
        data = JSON.parse(data);
        assert.equal(
          data.url,
          wptServer +
            "waterfall.php?test=120816_V2_2&run=1&cached=0&type=connection&mime=1&width=640&max=9&requests=1%2C2%2C4%2C6-8&cpu=0&bw=0&dots=0&labels=0"
        );
        done();
      }
    );
  ...
  it("gets a screenshot thumbnail input returns the API url", function (done) {
    exec(mock("screenshot 120816_V2_2 --thumbnail"), function (err, data) {
      ...
      assert.equal(
        data.url,
        wptServer + "thumbnail.php?test=120816_V2_2&file=1_screen.jpg&run=1&cached=0"
      );
      done();
    });
  });
