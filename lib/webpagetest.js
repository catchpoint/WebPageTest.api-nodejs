...
function get(pathname, callback, encoding) {
  ...
  (this.config.https ? https : http).get(options, function getResponse(res) {
    var data = '';
...
function dryRun(pathname) {
  return {
    url: url.format({
      ...
      port: (this.config.port !== 80 && this.config.port !== 443 ?
...
function WebPageTest(server, key) {
  ...
  // normalize hostname
  server = server || WebPageTest.defaultServer;
  if (!reProtocol.test(server)) {
    server = 'http://' + server;
  }
  server = url.parse(server);

  isHttps = server.protocol === 'https';
  this.config = {
    https: isHttps,
    ...
    key: key,
    port: server.port || (isHttps ? 443 : 80)
  };
}
