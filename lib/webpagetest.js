...
var testParams = {
  ...
  disableJavaScript: {name: 'noscript', bool: true},
  ignoreSSL: {name: 'ignoreSSL', bool: true},
  disableCompatibilityView: {name: 'standards', bool: true},
  saveResponseBodies: {name: 'bodies', bool: true},
  keepOriginalUserAgent: {name: 'keepua', bool: true},
  minimumDuration: {name: 'time'},
  blockAds: {name: 'blockads', bool: true},
  aftRenderingTime: {name: 'aft', bool: true},
...
function api(pathname, callback, query, options) {
  ...
  if (options.dryRun) {
    ...
    callback.apply(this,
      [undefined, helper.dryRun(config, pathname)].concat(options.args)
    );

  } else {

    // make the real API call
    get.call(this, config, pathname, function apiCallback(err, data, type) {
      if (!err) {
        try {
          if (options.parser) {
...
WebPageTest.testParams = testParams;
WebPageTest.defaultServer = 'http://www.webpagetest.org';
WebPageTest.defaultListenPort = 7719;
...
WebPageTest.prototype = {
  ...
  listen: function serverListen(port) {
    server.listen.call(this, port || WebPageTest.defaultListenPort);
  }
};
