...
var reSpace = /\s/,
    ...
    reResultsOptions = /(?:m|median) (\w+)/; // m TTFT or median render
...
function runTest(what, options, callback) {
  ...
  function poll(err, data) {
    if (!err && (!data || data && data.response &&
      ...
      polling = setTimeout(getTestResults.bind(this, testId,
        options.resultsOptions, poll.bind(this)), options.pollResults);
    } else {
  ...
  if (options.resultsOptions && typeof options.resultsOptions === 'string') {
    options.resultsOptions = reResultsOptions.exec(options.resultsOptions);
    if (options.resultsOptions) {
      options.resultsOptions = {medianMetric: options.resultsOptions[1]};
    }
  }
    ...
    server = http.createServer(function(req, res) {
      ...
      if (uri.pathname === '/testdone' && uri.query.id === testId) {
        server.close(getTestResults.bind(this, uri.query.id,
          options.resultsOptions, resultsCallback));
      }
    }.bind(this));
...
// Version
Object.defineProperty(WebPageTest, 'version', {value: '0.0.4'});
